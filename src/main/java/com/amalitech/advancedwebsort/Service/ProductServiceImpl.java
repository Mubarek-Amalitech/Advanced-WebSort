...
public class ProductServiceImpl implements  ProductService {
    ...
    public Product createProduct(ProductRequest productRequest) {
        Optional<Product> productCheck= productRepository.findByProductId(productRequest.productId());
        if (productCheck.isPresent() ) {
            throw  new RuntimeException(" product already exist");
        }
        Product product = Product.builder().name(productRequest.name()).quantity(productRequest.quantity())
                .productId(productRequest.productId()).build();
